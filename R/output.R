print_stats <- function(data) {
    ...
    cat(formatc("Univariate Analysis", width1), "\n\n", formatl("N"), formatr(data$Observations, 
        n), formats(), formatl("Variance"), formatr(data$Variance, n), "\n", formatl("Missing"), 
        formatr(data$Missing_Obs, n), formats(), formatl("Std Deviation"), formatr(data$Standard_Deviation, 
            n), "\n", formatl("Mean"), formatr(data$Mean, n), formats(), formatl("Range"), 
        formatr(data$Range, n), "\n", formatl("Median"), formatr(data$Median, n), 
        formats(), formatl("Interquartile Range"), formatr(data$Interquartile_Range, 
            n), "\n", formatl("Mode"), formatr(data$Mode, n), formats(), formatl("Uncorrected SS"), 
        formatr(data$Uncorrect_SS, n), "\n", formatl("Trimmed Mean"), formatr(data$Trimmed_mean, 
            n), formats(), formatl("Corrected SS"), formatr(data$Correct_SS, n), 
        "\n", formatl("Skewness"), formatr(data$Skewness, n), formats(), formatl("Coeff Variation"), 
        formatr(data$Coefficient_Of_Variation, n), "\n", formatl("Kurtosis"), formatr(data$Kurtosis, 
            n), formats(), formatl("Std Error Mean"), formatr(data$Std_Error_Mean, 
            n), "\n\n", formatc("Quantiles", width1), "\n\n", formatc("Quantile", 
            width2), formatc("Estimate", width2), "\n\n", formatc("100% Max  ", 
            width2), formatnc(data$Max, width2), "\n", formatc("99%       ", width2), 
        formatnc(data$Percentile_99, width2), "\n", formatc("95%       ", width2), 
        formatnc(data$Percentile_95, width2), "\n", formatc("90%       ", width2), 
        formatnc(data$Percentile_90, width2), "\n", formatc("75% Q3    ", width2), 
        formatnc(data$Percentile_75, width2), "\n", formatc("50% Median", width2), 
        formatnc(data$Median, width2), "\n", formatc("25% Q1    ", width2), formatnc(data$Percentile_25, 
            width2), "\n", formatc("10%       ", width2), formatnc(data$Percentile_10, 
            width2), "\n", formatc("5%        ", width2), formatnc(data$Percentile_05, 
            width2), "\n", formatc("1%        ", width2), formatnc(data$Percentile_01, 
            width2), "\n", formatc("0% Min    ", width2), formatnc(data$Min, width2), 
        "\n\n", formatc("Extreme Observations", width1), "\n\n", formatc("Lowest", 
            width2), formatc("Highest", width2), "\n\n", formatol("Value", ol), 
        format_gap(gap), formatol("Obs", ol), formats(), formatol("Value", ol), 
        format_gap(gap), formatol("Obs", ol), "\n")
    for (i in seq_len(5)) {
        cat("", formatol(data$Lowest_Obs[i], ol), format_gap(gap), formatol(data$Lowest_Obs_Index[i], 
            ol), formats(), formatol(data$Highest_Obs[i], ol), format_gap(gap), 
            formatol(data$Highest_Obs_Index[i], ol), "\n")
    }
...
|---------------------------------------------------------------------------|")
    for (i in seq_len(data$bins)) {
        k <- i + 1
        cat("\n|", formata(data$breaks[i], 1, 5), "-", formata(data$breaks[k], 
            1, 5), "|", formata(data$frequency[i], 2, 12), "|", formata(data$cumulative[i], 
            2, 12), "|", formatas(data$percent[i], 2, 12), "|", formatas(data$cum_percent[i], 
            2, 12), "|")
        cat("\n|---------------------------------------------------------------------------|")
    }
...
print_screen <- function(x) {
    ...
    columns <- c("  Column Name  ", "  Data Type  ", "  Levels  ", "  Missing  ", 
        "  Missing (%)  ")
    len_col <- as.vector(sapply(columns, nchar))
    xlev <- lapply(k$levels, paste, collapse = " ") %>% lapply(nchar) %>% unlist %>% 
        max
